...
0.4.1 (2026-07-24)
    ...
    pinned 3.12 -> 3.10 (registry_version 0.1.0 -> 0.2.0). Decision: the
...
0.4.0 (2026-07-24)
    ...
    abstract name property (ADR-001), every subclass not implementing it
...
0.3.0 (2026-07-24)
    ...
    * EXC: new exclude_modules registry key (default: _version.py) for
...
0.1.0 (2026-07-24)
    ...
    derived from the ADR series; stdlib-only, single file, JSON registry
...
Pattern mapping (dogfooding):
    ...
    each rule         -> IStrategy   (execute(caller, **kwargs); rules are
                                      interchangeable and registry-driven)
...
Rules (mapped to the ADR series):
  R-CORE-IMP-01  interface modules import only the allowlist:
                 __future__, abc, typing, typing_extensions, datetime,
                 collections.abc + package-relative.          [ADR-003/005]
  R-CORE-IMP-02  absolute imports resolve within pinned stdlib or the
                 allowed third-party set.                     [ORG-01]
  R-CORE-SFX-01  no module-level side effects.                [ADR-005]
  R-CORE-ABS-01  I<Upper>-named class must be abstract.       [ORG-02d]
  R-CORE-ABS-02  abstract class should carry the I prefix (warning).
  R-CORE-STA-01  no __init__ in interface classes.            [ADR-001]
  R-CORE-STA-02  __slots__, if declared, is empty.            [ADR-002]
...
results admit counting, not weighted sums). Exit: 0 clean, 1 errors,
            ...
            for name, line in scan.typevars.items():
                if name in forbidden or len(name) <= 2:
                    ...
                    log.add(
                        ...
                        scan.relname,
                        line,
                        f"TypeVar {name!r} not in the role vocabulary — extend the registry via ADR or rename",
                    )
            ...
            for node in scan.tree.body:
                    ...
                    if isinstance(item, (ast.FunctionDef, ast.AsyncFunctionDef)):
                        ...
                        if item.name == "__init__":
                            log.add(
                                ...
                                f"constructor discipline (ADR-001)",
                            )
                            ...
                            if isinstance(tgt, ast.Name) and tgt.id == "__slots__":
                                ...
                                if not empty:
                                    log.add(
                                        ...
                                        scan.relname,
                                        item.lineno,
                                        f"{node.name}.__slots__ non-empty: interfaces contribute no storage (ADR-002)",
                                    )
                if concrete and _is_interface_name(node.name):
                    log.add(
                        ...
                        f"defining-algorithm exception (ADR-001); not "
                        f"machine-decidable",
                    )
...
class AbstractnessRule(LintRule):
    ...
    construct. ABS-03 restores it: a class whose only abstract members
    are inherited from the root, and which defines concrete machinery of
    its own, is concrete in spirit regardless of isabstract.
    """
    ...
    def execute(self, caller: IWattleflow, **kwargs) -> Any:
        ...
        try:
            ...
            pass  # pre-refactor root or missing module; ABS-01 still valid
...
class CoreLintRun(WemComponent, ITemplate):
    ...
    courtesy of ITemplate.process()'s try/finally.
    """
    ...
    def __init__(self, pkg_dir: Path, registry: Optional[Path], static_only: bool) -> None:
    ...
    def initialise(self) -> None:
        ...
        if self._registry is not None:
            try:
                override = json.loads(self._registry.read_text(encoding="utf-8"))
                self._cfg.update(override)
            except (OSError, json.JSONDecodeError) as exc:
                print(f"error: cannot read registry {self._registry}: {exc}", file=sys.stderr)
                self.exit_code = 2
                raise SystemExit(2)
        ...
        if running != self._cfg["python_reference"]:
            self._log.add(
                ...
                "(environment)",
                0,
                f"running Python {running}, registry pins "
        ...
        for path in sorted(self._pkg_dir.glob("*.py")):
            if path.name in excluded:
                self._log.add(
                    ...
                    path.name,
                    0,
                    "excluded by registry (declared blind spot: infrastructure module, not an interface module)",
                )
                continue
    ...
    def finalise(self) -> None:
        print(ReportBuilder(self._log).build())
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument("--registry", type=Path, default=None, help="JSON registry override for embedded defaults")
    ...
    run = CoreLintRun(pkg_dir, args.registry, args.no_import)
    run.process()
